...
public class BookingControllerTest {
  ...
  @MockitoBean
  private BookingDriverApplicationService bookingDriverApplicationService;
  ...
  void testGetBookingById_HttpNotFound() throws Exception {
    when(bookingService.getBookingById(99L)).thenThrow(new IllegalArgumentException("Booking not found"));

    mockMvc.perform(get("/booking/v1/{id}", 99L))
      .andExpect(status().isBadRequest());
  }

  @Test
  void testGetBookingById_InvalidIdFormat() throws Exception {
    mockMvc.perform(get("/booking/v1/abc"))
      .andExpect(status().isBadRequest());
  }
  ...
  void testDeleteBooking_HttpConflict_ActiveBooking() throws Exception {
    doThrow(new IllegalStateException("Cannot delete active booking")).when(bookingService).deleteBooking(1L);
  ...
  void testOptionsRequest() throws Exception {
    mockMvc.perform(options("/booking/v1"))
      .andExpect(status().isOk());
  }
