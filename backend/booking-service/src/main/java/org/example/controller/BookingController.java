...
public class BookingController {
  ...
  public ResponseEntity<BookingResponse> createBooking(
    ...
    return ResponseEntity
      .created(URI.create("/booking/v1/" + booking.getId()))
  ...
  public ResponseEntity<BookingResponse> submitBooking(@PathVariable Long id) {
    return ResponseEntity.ok(
      toResponse(bookingService.submitBooking(id))
    );
  }
  ...
  public ResponseEntity<BookingResponse> confirmBooking(@PathVariable Long id) {
    return ResponseEntity.ok(
      toResponse(bookingService.confirmBooking(id))
    );
  }
  ...
  public ResponseEntity<BookingResponse> cancelBooking(@PathVariable Long id) {
    return ResponseEntity.ok(
      toResponse(bookingService.cancelBooking(id))
    );
  }
  ...
  public ResponseEntity<BookingResponse> completeBooking(@PathVariable Long id) {
    return ResponseEntity.ok(
      toResponse(bookingService.completeBooking(id))
    );
  }
