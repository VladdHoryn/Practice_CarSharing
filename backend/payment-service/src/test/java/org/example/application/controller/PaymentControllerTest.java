...
class PaymentControllerTest {
  ...
  class GetById {
    ...
    void shouldReturn5xxWhenNotFound() throws Exception {
      when(paymentService.getById(99L)).thenThrow(new EntityNotFoundException("Payment not found"));
      mockMvc.perform(get("/payment/v1/99"))
  ...
  class CreatePayment {
    ...
    private CreatePaymentRequest validRequest() {
      return new CreatePaymentRequest(10L, BigDecimal.valueOf(500), PaymentMethod.CARD, "USD");
    }
  ...
  class UpdatePayment {
    ...
    void shouldUpdatePaymentForAdmin() throws Exception {
      ...
      UpdatePaymentRequest req = new UpdatePaymentRequest(BigDecimal.valueOf(600), PaymentMethod.GOOGLE_PAY, "EUR");
    ...
    @DisplayName("RENTER отримує 403")
    void shouldReturn403ForRenter() throws Exception {
      UpdatePaymentRequest req = new UpdatePaymentRequest(BigDecimal.valueOf(600), null, null);
      mockMvc.perform(put("/payment/v1/1")
          .with(csrf())
          .contentType(MediaType.APPLICATION_JSON)
          .content(objectMapper.writeValueAsString(req)))
  ...
  class DeletePayment {
    ...
    void shouldDeletePaymentForAdmin() throws Exception {
      ...
      mockMvc.perform(delete("/payment/v1/1").with(csrf()))
  ...
  class RefundPayment {
    ...
    void shouldRefundForRenter() throws Exception {
      ...
      mockMvc.perform(patch("/payment/v1/1/refund").with(csrf()))
        .andExpect(status().isOk());
    }
  ...
  class ChangeStatus {
    ...
    void shouldChangeStatusForAdmin() throws Exception {
      ...
      mockMvc.perform(post("/payment/v1/1/status/change")
          .with(csrf())
          .contentType(MediaType.APPLICATION_JSON)
          .content(objectMapper.writeValueAsString(req)))
        .andExpect(status().isNoContent());
