...
import org.example.repository.UserRepository; // Припускаємо наявність UserRepository
...
public class UserDocumentApplicationService {
  ...
  @Transactional
  public UserDocument uploadDocument(Long userId, DocumentType documentType, byte[] fileData, String contentType, String originalFileName) {
    User user = userRepository.findById(userId)
      .orElseThrow(() -> new IllegalArgumentException("User not found with id: " + userId));

    UserDocument document = documentRepository.findByUserIdAndDocumentType(userId, documentType)
      .orElseGet(() -> {
  ...
  public boolean areAllDocumentsVerifiedAndPresent(Long userId) {
    ...
    Set<DocumentType> verifiedTypes = userDocuments.stream()
