...
class UserControllerTest {
    ...
    class GetAllUsers {
        ...
        void shouldReturn401ForAnonymous() throws Exception {
            mockMvc.perform(get("/user/v1")).andExpect(status().is4xxClientError());
        }
    }
    ...
    class GetUserByKeycloakId {
        ...
        void shouldReturnUserForAdmin() throws Exception {
            ...
            mockMvc.perform(get("/user/v1/keycloak/kc-id-other")).andExpect(status().isOk());
        }
        ...
        void shouldReturn403ForDifferentUser() throws Exception {
            mockMvc.perform(get("/user/v1/keycloak/kc-id-123")).andExpect(status().isForbidden());
        }
    ...
    class UpdateUser {
        ...
        void shouldUpdateUserForOwner() throws Exception {
            ...
            mockMvc.perform(
                            put("/user/v1/keycloak/kc-id-123")
                                    .with(csrf())
                                    .contentType(MediaType.APPLICATION_JSON)
                                    .content(objectMapper.writeValueAsString(validRequest())))
                    .andExpect(status().isOk())
        ...
        void shouldUpdateUserForAdmin() throws Exception {
            ...
            mockMvc.perform(
                            put("/user/v1/keycloak/kc-id-other")
                                    .with(csrf())
                                    .contentType(MediaType.APPLICATION_JSON)
                                    .content(objectMapper.writeValueAsString(validRequest())))
                    .andExpect(status().isOk());
    ...
    class DeactivateUser {
        ...
        void shouldDeactivateUserForAdmin() throws Exception {
            UserResponse deactivated =
                    UserResponse.builder().id(1L).keycloakId("kc-id-123").isActive(false).build();
    ...
    class CountUsersByRole {
        ...
        void shouldCountOwnersForAdmin() throws Exception {
            ...
            mockMvc.perform(get("/user/v1/analytics/admin/roles/count").param("role", "OWNER"))
                    .andExpect(status().isOk())
