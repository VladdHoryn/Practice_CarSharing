...
class UserControllerTest {
  ...
  class GetUserByKeycloakIdTests {
    ...
    @Test
    @DisplayName("200 для ADMINISTRATOR")
    @WithMockUser(username = "someone-else", roles = {"ADMINISTRATOR"})
    void shouldReturnOk_WhenAdmin() throws Exception {
    ...
    @WithMockUser(username = "kc-id-123", roles = {"RENTER"})
    ...
    @WithMockUser(username = "another-user", roles = {"RENTER"})
  ...
  class ActivateUserTests {
    ...
    @WithMockUser(roles = {"ADMINISTRATOR"})
    void shouldReturnOk_WhenAdmin() throws Exception {
      when(userApplicationService.activateUser("kc-id-123"))
        .thenReturn(sampleUserResponse());
  ...
  class CountUsersByRoleTests {
    ...
    void shouldReturnOk_WhenAdmin() throws Exception {
      ...
      mockMvc.perform(
          get("/user/v1/analytics/admin/roles/count")
            .param("role", "RENTER"))
        .andExpect(status().isOk())
