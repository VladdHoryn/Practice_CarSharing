...
class GatewayRoutingTest {
    ...
    void allRoutesShouldHavePredicates() {
        routeLocator
                .getRoutes()
                .doOnNext(route -> assertThat(route.getPredicate()).isNotNull())
                .collectList()
                .block();
    }
    ...
    void userServiceRoute_shouldHavePathPredicate() {
        routeLocator
                ...
                .next()
                .doOnNext(
                        route -> assertThat(route.getPredicate().toString()).contains("/user/v1/"))
                .block();
    }
    ...
    void carServiceRoute_shouldHavePathPredicate() {
        routeLocator
                ...
                .next()
                .doOnNext(route -> assertThat(route.getPredicate().toString()).contains("/car/v1/"))
                .block();
    }
    ...
    void bookingServiceRoute_shouldHaveCorrectUri() {
        routeLocator
                ...
                .filter(route -> "booking-service".equals(route.getId()))
                .next()
                .doOnNext(route -> assertThat(route.getUri().toString()).contains("localhost:9998"))
                .block();
    }
    ...
    void carServiceRoute_shouldHaveCorrectUri() {
        routeLocator
                ...
                .filter(route -> "car-service".equals(route.getId()))
                .next()
                .doOnNext(route -> assertThat(route.getUri().toString()).contains("localhost:9997"))
                .block();
    }
    ...
    void paymentServiceRoute_shouldHaveCorrectUri() {
        routeLocator
                ...
                .filter(route -> "payment-service".equals(route.getId()))
                .next()
                .doOnNext(route -> assertThat(route.getUri().toString()).contains("localhost:9996"))
                .block();
    }
    ...
    void shouldNotHaveUnknownRoutes() {
        List<String> knownIds =
                List.of("user-service", "booking-service", "car-service", "payment-service");

        routeLocator
                .getRoutes()
                .doOnNext(route -> assertThat(knownIds).contains(route.getId()))
                .collectList()
                .block();
    }
    ...
    void userServiceRoute_shouldHaveCorrectUri() {
        routeLocator
                ...
                .filter(route -> "user-service".equals(route.getId()))
                .next()
                .doOnNext(route -> assertThat(route.getUri().toString()).contains("localhost:9999"))
                .block();
    }
}
