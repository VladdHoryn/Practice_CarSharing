...
class AnalyticsAggregatorApplicationServiceTest {
  ...
  class GetOwnerAnalyticsSummary {
    ...
    @Test
    @DisplayName("fallback: countCompletedBookingsByOwnerId кидає виняток -> completedBookings = 0")
    ...
    @Test
    @DisplayName("fallback: sumTotalPriceByOwnerIdAndStatus кидає виняток -> totalRevenue = ZERO")
    ...
    @Test
    @DisplayName("усі клієнти впали одночасно -> усі поля на дефолтах, виняток не пробивається назовні")
    ...
    @Test
    @DisplayName("Feign повертає 200 з null тілом (не виняток) -> у DTO потрапляє null, без fallback")
  ...
  class GetAdminAnalyticsSummary {
    ...
    void returnsFullyPopulatedSummary_whenAllClientsSucceed() {
      ...
      when(
        bookingServiceClient.sumLastMonthRevenue(
          TOKEN, COMPLETED_STATUS, PERIOD_START))
    ...
    @Test
    @DisplayName("fallback: countUsersByRole(OWNER) кидає виняток -> totalOwners = 0, totalRenters рахується нормально")
    ...
    @Test
    @DisplayName("fallback: countBookingsByDayOfWeek кидає виняток -> bookingsByDayOfWeek = null")
